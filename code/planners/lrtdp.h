 ...
 public:
  ...
  action_t const* decideAction(state_t const& s) override {
    if (isSolved(s)) {
      ...
      // whole problem, just do one trial (check-solved inclused).
      trial(s);
  ...
  void setPrintFinishedInfo(bool const val) {
    print_finished_info_ = val;
  }

 private:
  /*
   * New sample method to handle modified SSPs (fret-s3p)
   */
  state_t sampleOutcome(state_t const& s, action_t const& a) {
    static ProbDistStateHash pr_hash;
    ssp_.expand(a, s, pr_hash);
    double total_mass = 1.0;
    double r = rand0toN_d(total_mass);
    for (auto const& ip : pr_hash) {
      r -= ip.prob();
      if (r <= 0) {
//        std::cout << "LRTDP::sampleOutcome = "
//                  << ip.event().toStringFull(gpt::problem,0) << std::endl;
        return ip.event();
      }
    }
    std::cout << "LRTDP::sampleOutcome went wrong and this shouldn't be reachable. "
              << "Remaining prob. mass r = " << std::scientific << r << std::fixed << "\n";
    return pr_hash.begin().event();
  }
  ...
  bool print_finished_info_ = true;
};
