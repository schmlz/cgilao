...
#endif
void __my_warning_function__(char const* f, int l, std::string frm, ...);
...
void __my_debug_manager__(char const* f, int l, std::string const& t,
...
inline std::string printUsecForHumans(uint64_t time_usec) {
  std::ostringstream ost;
  constexpr uint64_t sec = 1000000;
  constexpr uint64_t mn = 60 * sec;
  constexpr uint64_t hour = 60 * mn;
  if (time_usec > hour) {
    ost << (time_usec / hour) << "h ";
    time_usec = time_usec % hour;
  }
  ost << (time_usec / mn) << "m ";
  time_usec = time_usec % mn;
  ost << (time_usec / sec) << "s ";
  time_usec = time_usec % sec;
  ost << (time_usec / 1000) << "ms";
  return ost.str();
}

std::string humanReadableTimeIntervalSince(uint64_t start);
...
inline uint64_t get_cputime_usec() {
  ...
  { (void) sizeof(rt); }  // To avoid gcc to complain
  cputime += 1000000 * rusage.ru_utime.tv_sec + rusage.ru_utime.tv_usec;
  return cputime;
#else
...
inline bool runForUsec(uint64_t max_time_usec, std::function<void(void)> f)
{
  ...
  SystemResourcesDeadline my_deadline(max_time_usec, gpt::max_rss_kb);
  if (!gpt::setDeadline(&my_deadline)) {
    std::cerr << "Deadline already set and runForUsec need to set a new "
              << "deadline" << std::endl;
    exit(-1);
  }
  try { f(); }
...
/*
 * FWT: function to help debugging the probability_of_adding_atoms function
 * for the determinizationless atom heuristic
 */
