...
LMCutHeuristic::LMCutHeuristic(problem_t const& problem, size_t cost_idx)
  : determinizationBasedAtomHeuristic(problem, cost_idx, false),
    prec_op_vbits_(operator_ptr_.size()), adds_op_vbits_(operator_ptr_.size())
{
  name_ = "lm-cut";
//  std::cout << "[LMCut] number of atoms in the problem: "
//            << problem.number_atoms() << std::endl;

  // if (problem.number_atoms() > LMCUT_MAX_ATOMS) {
  //   std::cout << "The hardcoded size of VectorOfBitsAtm ("
  //             << LMCUT_MAX_ATOMS << ") is too small for atoms. Increase it to "
  //             << "at least " << problem.number_atoms() << ". Quitting"
  //             << std::endl;
  //   exit(240);
  // }

  // if (operator_ptr_.size() > LMCUT_MAX_ACTIONS) {
  //   std::cout << "The hardcoded size of VectorOfBits ("
  //             << LMCUT_MAX_ACTIONS << ") is too small for operator_ptr_. "
  //             << "Increase it to at least " << operator_ptr_.size()
  //             << ". Quitting" << std::endl;
  //   exit(240);
  // }
  ...
  for (size_t i = 0; i < operator_ptr_.size(); ++i) {
    ...
    prec_op_vbits_[i] = AtomBitset(problem.number_atoms());
    ...
    adds_op_vbits_[i] = AtomBitset(problem.number_atoms());
...
double LMCutHeuristic::computeLMCut(state_t const& s,
...
//  std::cout << "[LMCut] =======" << std::endl;
  ...
  VectorOfBits allowed_actions(operator_ptr_.size());

  double total_cost = 0;
  VectorOfBitsAtm ext_goal_set(problem_.number_atoms());  // Declared here to avoid alloc/dealloc
  size_t ite = 0;
...
//        std::cout << "[LMCut] Forbidding op " << ia << std::endl;
    ...
    VectorOfBits cut = findCut(ext_goal_set, action_cost);
    assert(cut.count() > 0);
...
VectorOfBits LMCutHeuristic::findCut(VectorOfBitsAtm const& ext_goal_set,
                                  std::vector<double> const& action_cost) const
{
  // FANCY_DIE_IF(LMCUT_MAX_ACTIONS < operator_ptr_.size(), 666,
  //     "The hardcoded size of VectorOfBits is (%d) too small. "
  //     "Increase it to at least %d", LMCUT_MAX_ACTIONS, operator_ptr_.size());

  VectorOfBits cut(operator_ptr_.size());  // Default is zero
  assert(cut.count() == 0);
...
void LMCutHeuristic::computeCostOfAtomsForConstrainedActionSet(state_t const& s,
    std::vector<double> const& action_cost,
    VectorOfBits const* allowed_actions)
{
  ...
  static VectorOfBits aa(operator_ptr_.size());
  aa.reset();
