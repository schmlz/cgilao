...
(define (domain tire)
  ...
  (:action move-car
    ...
    :effect (and (decrease reward 0) (vehicle-at ?to) (not (vehicle-at ?from)) (probabilistic 2/5 (not (not-flattire))))
  )
  (:action loadtire
    :parameters (?loc - location)
    :precondition (and (vehicle-at ?loc) (spare-in ?loc))
    :effect (and (decrease reward 0) (hasspare) (not (spare-in ?loc)))
  )
  (:action changetire
    :precondition (hasspare)
    :effect (and (decrease reward 0) (probabilistic 1/2 (and (not (hasspare)) (not-flattire))))
  )
)
