...
(:action explore
    ...
    :effect (and (decrease reward 0) (explored ?loc) (probabilistic
        0.7 (landable ?loc)))
)
...
(:action land-base
    ...
    :effect (and (decrease reward 0) (on-ground) (human-rescued))
)

(:action land
    :parameters (?loc - zone)
    :precondition (and (at ?loc) (not (on-ground)) (not (= ?loc base)) (human-alive) (landable ?loc))
    :effect (and (decrease reward 0) (on-ground) (probabilistic
...
(:action takeoff
    ...
    :effect (and (decrease reward 0) (not (on-ground)))
)
