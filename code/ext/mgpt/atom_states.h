...
#if defined(USE_KOMIHASH)
#include "../../ext/faster_hashing/komihash.h"
#endif
...
class state_t
{
 ...
 public:
  ...
  // Make a state from a formula [at l-001-l001, broken,...] where each
  // active_atoms[i] is the string representation of an atom without
  // square brackets, parentheses, etc.
  state_t(std::vector<std::string> act_atoms) : data_(new unsigned[size_]()) {
    initializeUsingVectorOfNamedAtoms(act_atoms);
  }
  ...
  void initializeUsingVectorOfNamedAtoms(std::vector<std::string> active_atoms);
  ...
  {
#if defined(USE_KOMIHASH)
    return komihash(data_, size_ * sizeof(unsigned), gpt::seed);
#endif
#if 1
    unsigned *ptr, result;
  ...
  bool operator==(state_t const& state) const {
    // FWT: size_ is static for now
    assert(size_ == state.size_);
    // FWT: it should work fine with new because it is primitive type
    return ! memcmp(data_, state.data_, size_ * sizeof(unsigned));
  }
...
#if 0
    size_t i = atom % 32;
    unsigned data = data_[atom>>5];
    for( size_t j = 0; j < i; data = (data>>1), ++j );
    return( data % 2 );
#else
    size_t i = atom >> 5;  // i = (int) (atom / 32) with no rounding
    size_t j = atom % 32;
...
//    nstate->make_digest();
  ...
  std::string toStringFullWithNegations(problem_t const* problem, bool print_idx,
  ...
  class const_predicate_iterator
  {
    ...
    void first( void )
    {
      ...
      if( idx_ < state_->size() )
      {
        unsigned data = state_->data()[idx_];
...
// TODO: FWT: Deprecate this in favor of the std::hash below
struct hashState {
