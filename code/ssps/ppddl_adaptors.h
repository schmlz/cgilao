...
class SSPfromPPDDL : public SSPIface {
 public:
  SSPfromPPDDL(problem_t const& problem) : p_(problem) {
    if (p_.constraints().size() > 0) {
      std::cout << "[WARNING] The problem '" << name() << "' is a Constrained "
        << "SSP but the constraints are being ignored." << std::endl;
    }
  }
...
/*
 * Adaptor class from problem_t to ConstrSSPIface
 */
class ConstrSSPfromPPDDL : public ConstrSSPIface {
 public:
  ConstrSSPfromPPDDL(problem_t const& problem) : p_(problem) { }
  ...
  ConstraintMap const& constraints() const override { return p_.constraints(); }

  size_t numCostFunctions() const override {
    return p_.domain().functions().size();
  }
