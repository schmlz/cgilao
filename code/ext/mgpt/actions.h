...
class conditionalEffectList_t
{
  ...
  VecRationals costVector(state_t const& s) const;
...
class probabilisticEffectList_t
{
  ...
  VecRationals costVector() const;

  Rational cost(state_t const& s, size_t cost_idx = ACTION_COST) const;
  VecRationals costVector(state_t const& state) const;
...
class stripsEffect_t : public effect_t
{
 ...
 public:
  ...
  void increaseCostVecBy(VecRationals const& v) {
    assert(v.size() == cost_.size());
  ...
  VecRationals costVector() const { return cost_; }
...
class conditionalEffect_t : public effect_t
{
  ...
  VecRationals costVector(state_t const& s, bool& prec_holds) const {
    static const VecRationals all_zeros(s_effect_.costVector().size(), 0);
    if (precondition().holds(s)) {
      prec_holds = true;
      return s_effect_.costVector();
    }
    else {
      prec_holds = false;
      return all_zeros;
...
class deterministicEffect_t : public effect_t
{
 ...
 public:
  ...
  VecRationals costVector(state_t const& s) const {
    VecRationals rv = s_effect_.costVector();
    VecRationals cond_costs = c_effect_.costVector(s);
    for (size_t i = 0; i < cond_costs.size(); ++i) {
      rv[i] += cond_costs[i];
    }
    return rv;
  }
  ...
  VecRationals costVector() const {
    // FWT: Same as above, i.e., cost(size_t)
    return s_effect_.costVector();
  }
...
class action_t
{
 ...
 public:
  ...
  virtual VecRationals costVector(state_t const& state) const = 0;

  virtual Rational cost(size_t cost_idx) const = 0;
  virtual Rational cost() const { return cost(ACTION_COST); }
  virtual VecRationals costVector() const = 0;
...
class deterministicAction_t : public action_t
{
 ...
 public:
  ...
  virtual VecRationals costVector() const;

  virtual Rational cost(state_t const& state, size_t cost_idx) const;
  virtual VecRationals costVector(state_t const& s) const;
...
inline VecRationals probabilisticEffectList_t::costVector() const {
  assert(size() > 0);
  VecRationals expected_cost = effect(0).costVector();
  if (size() > 0) {
    for (Rational& c : expected_cost) {
      c *= effect(0).probability();
    }
    for (size_t i = 1; i < size(); ++i) {
      VecRationals c_i = effect(i).costVector();
      for (size_t j = 0; j < expected_cost.size(); ++j) {
        expected_cost[j] += effect(i).probability() * c_i[j];
      }
    }
  }
  return expected_cost;
