...
std::ostream& operator<<(std::ostream& out, RoundSummary const& r) {
  out << "[Round Summary]: status = " << r.exitStatus << "\n"
      << "[Round Summary]: # of actions = " << r.totalActionsApplied << "\n"
      << "[Round Summary]: CPU+System time = " << r.totalCpuPlusSystemTime
      << std::endl;
  FunctionTable const& ft = gpt::problem->domain().functions();
  assert(r.accumulatedCost.size() == ft.size());
  for (size_t k = 0; k < ft.size(); ++k) {

    out << "[Round Summary]: Total ";
    if (ft.name(k) == "reward") {
      out << "COST";
    }
    else {
    ...
    out << " = " << r.accumulatedCost[k] << std::endl;
  }
  return out;
}
...
// static
Simulator* createSimulator(ConstrSSPIface const& cssp, std::string const& name) {
  Simulator* simulator = NULL;
  if (!strcasecmp(name.c_str(), "local")) {
    simulator = new LocalSimulator(cssp);
  }
    ...
    Planner* planner, uint32_t max_turn, uint64_t max_cpu_sys_time_usec)
{
  ...
  size_t const num_cost_f = gpt::problem->domain().functions().size();

  RoundSummary r(num_cost_f, 0, 0, EndOfRoundStatus::PLANNER_GAVE_UP);
  ...
  try {
    while (true) {
      ...
      if (cssp_.isGoal(cur_s)) {
        if (output_turns_) {
          std::cout << "T[" << r.totalActionsApplied << "] GOAL = "
                    << cur_s.toStringFull(gpt::problem)
                    << std::endl << std::endl;
        }
        r.accumulatedCost[ACTION_COST] += cssp_.terminalCost(cur_s);
      ...
      if (a == NULL) {
        ...
        if (gpt::use_action_cost) {
          ...
          // Adding C_k(s,a) for each k
          VecRationals const cost_vec_s_a = cssp_.costVector(cur_s,*a);
          for (size_t k = 0; k < cost_vec_s_a.size(); ++k) {
            r.accumulatedCost[k] += cost_vec_s_a[k];
          }
        }
        if (output_turns_) {
          ...
          if (gpt::use_action_cost) {
            ...
            for (size_t k = 0; k < cost_vec_s_a.size(); ++k) {
              if (k == ACTION_COST) {
                ...
                std::cout << " " << gpt::problem->domain().functions().name(k)
                          << " += ";
              }
              std::cout << cost_vec_s_a[k];
            }
            std::cout << std::endl;
          }
        }
  ...
  endRound(planner);
//  gpt::log.finishCurRound(goal_reached, t, accumulated_cost.double_value());
  return r;
}
