...
(define (domain exploding-blocksworld)
  ...
  (:action pick-up
   ...
   :effect (and (decrease reward 0) (holding ?b1) (clear ?b2) (not (emptyhand)) (not (on ?b1 ?b2)))
  )
  (:action pick-up-from-table
   :parameters (?b - block)
   :precondition (and (emptyhand) (clear ?b) (on-table ?b) (no-destroyed ?b))
   :effect (and (decrease reward 0) (holding ?b) (not (emptyhand)) (not (on-table ?b)))
  )
  (:action put-down-no-detonated
   :parameters (?b - block)
   :precondition (and (holding ?b) (no-destroyed-table) (no-detonated ?b))
   :effect (and (decrease reward 0) (emptyhand) (on-table ?b) (not (holding ?b))
  ...
  (:action put-on-block-no-detonated
   ...
   :effect (and (decrease reward 0) (emptyhand) (on ?b1 ?b2) (not (holding ?b1)) (not (clear ?b2))
