...
//      throw Exception( "sorry, no more atoms available" );
    ...
    if (atom_index_ > MAX_ATOMS) {
      std::cout << "This problem has more than " << MAX_ATOMS
                << "! Update MAX_ATOMS in global.h or recompile using "
...
void problem_t::parseConstraints() {
  FunctionTable const& ft = domain().functions();

  std::ifstream infile(gpt::__ppddl_problem_filename);
  const std::string constr_token = ";; CONSTR ";
  std::string line;

  while (std::getline(infile, line)) {
    if (line.substr(0, constr_token.size()) == constr_token) {
      std::string constraint = line.substr(constr_token.size());
      trim_string(constraint);
      std::deque<std::string> parts = splitString(constraint, " ");
      if (parts.size() != 4 || parts[1] != "<=") {
        std::cout << "[problem_t::parseConstraints] invalid constraint: '"
                  << constraint << "'. Quitting..." << std::endl;
        exit(-10);
      }
      else if (parts[0] == "reward") {
        std::cout << "[problem_t::parseConstraints] Error! The function '"
                  << parts[0] << "' cannot be used in constraints. Quitting..."
                  << std::endl;
        exit(-11);
      }
      int func_idx = -1;
      for (size_t i = 0; i < ft.size(); ++i) {
        if (ft.name(i) == parts[0]) {
          func_idx = i;
          break;
        }
      }
      if (func_idx == -1) {
        std::cout << "[problem_t::parseConstraints] Error! The function '"
                  << parts[0] << "' not found in the problem definition. "
                  << "Quitting..." << std::endl;
        exit(-12);
      }
      double max_expected_val = std::stof(parts[2]);
      double deadend_penalty = std::stof(parts[3]);
      std::cout << "[problem_t::parseConstraints] Found the constraint: ";
      constr_map_.insert(parts[0], func_idx, max_expected_val,
                         deadend_penalty);
      std::cout << parts[0] << " (func_idx = " << func_idx << ") <= "
                << max_expected_val << "  Dead-end penalty = "
                << deadend_penalty << std::endl;
    }
  }
  infile.close();
}
...
void problem_t::flatten() {
  ...
  if (constraints().size() > 0) {
    std::cout << "[problem_t::flatten] This problem is a Constrained SSP"
              << std::endl;
  }
  else {
    std::cout << "[problem_t::flatten] This problem is an SSP" << std::endl;
  }
        ...
        it != actions().end(); ++it)
  {
    ...
    if (pact) {
      _D(DEBUG_ACTION_FLATTENING, std::cout
                ...
                << (pact->is_reward_consistent() ? "yes" : "NO!")
                << std::endl;
      );

      DIE(pact->is_reward_consistent(),
...
//  }
  ...
  if (gpt::dynamic_deadend_value) {
    // Checking if there's an action that costs more than the value we will
    // assign to deadends
//    if ((cost_upperbound + gpt::cost_shift).double_value() >=
//                                        gpt::dead_end_value.double_value()) {
    Rational new_dead_end_value =
        (cost_upperbound + gpt::cost_shift)
          * Rational((int) (1 / gpt::dynamic_deadend_epsilon));
    std::cout
      << "[DYNAMIC DEADEND VALUE]: The value associated to a deadend ("
      << gpt::dead_end_value << ") was changed to "
      << new_dead_end_value
      << " (cost_upperbound = " << cost_upperbound
      << ", cost_shift = " << gpt::cost_shift
      << ", dynamic_deadend_epsilon = " << gpt::dynamic_deadend_epsilon
      << ")." << std::endl;
    gpt::dead_end_value = new_dead_end_value;
//      }
  }
  ...
  for (ai = problem.actionsT().begin(); ai != problem.actionsT().end(); ++ai) {
    ...
    if (cdact->effect().c_effect().size() == 0) {
      ...
      _D(DEBUG_MED_RELAX, std::cout
        ...
        cdact->print_full();
        std::cout << "\nThe cost of this action is: " << cdact->cost() << std::endl;
      )
      size_t i;
      tmp_set.clear();
      ssets.clear();
...
state_t const problem_t::get_initial_state() const {
  ...
  {
    // EDGE CASE: it can happen that at an atom appears no preconditions, effects, nor the goal,
    // but still appears in the initial state.
    //
    // In that case, the atom has not been created by flatten, and doing s0.add(**ai) would try to
    // create it which causes issues.
    //
    // SOLUTION: just ignore such atoms. They are not needed for the problem and should be pruned.
    //
    // Example:  canadian-nomystery-small  p01
    std::map<const Atom*,ushort_t>::const_iterator it = atom_hash_.find( *ai );
    if( it == atom_hash_.end() ) {
      continue;
    }
