    ...
    //, relaxation_(problem.strong_relaxation(use_self_loop_relaxation))
{
  if (!gpt::__strong_relaxation) {
    gpt::__strong_relaxation = &problem.strong_relaxation(use_self_loop_relaxation);
  }
  ...
  if (cost_idx_ != ACTION_COST) {
    // This cost function is not the reward (i.e., cost), so finding the correct
    // dead end penalty
    dead_end_value_ = -1.0;
    ConstraintMap const& constrs = problem.constraints();
    for (size_t i = 0; i < constrs.size(); ++i) {
      if (constrs.costIdx(i) == cost_idx_) {
        dead_end_value_ = constrs.deadendPenalty(i);
//        std::cout << "[determinizationBasedAtomHeuristic] "
//                  << "Using " << dead_end_value_ << " as dead end penalty for "
//                  << constrs.name(i) << " (cost_idx_ = "
//                  << cost_idx_ << ")" << std::endl;
        break;
      }
    }
