...
class ExternalDetPlannerInterface {
 public:
  ...
  static std::shared_ptr<ExternalDetPlannerInterface> createInterface(problem_t const& problem,
...
//    START_TIMING("det_planner_exec");
    DetPlannerReturnType r = _run(s, plan, extra_goals);
//    STOP_TIMING("det_planner_exec");
  ...
  virtual DetPlannerReturnType _run(state_t const& s,
                              std::vector<DetPlannerUnparsedAction>& plan,
                              std::vector<state_t> const* extra_goals) = 0;
...
//  problem_t const& problem() const { return problem_; }
  ...
  std::string tmpDir() const { return det_pddl_.tmp_dir; }
  std::string domainFile() const { return det_pddl_.domain_file_path; }
