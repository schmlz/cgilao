  ...
  : data_(new unsigned[size_]())
{
  ...
  if (use_atom_index) {
    ...
    for (std::string& id : active_atoms) {
      ...
      assert(gpt::problem->domain().predicates().arity(atom_predicate)
              == term_parameters.size());
...
std::string state_t::toStringFullWithNegations(problem_t const* problem, bool print_idx,
    bool print_braces) const
{
  std::ostringstream ost;
  state_t::const_predicate_iterator ai;
  ...
  for (int i = 0; i < problem->number_atoms(); ++i) {
    if (i % 2 == 1) { continue; }

    ost << " ";
    const Atom *atom = problem_t::atom_inv_hash_get( i );
    if (print_idx)
      ost << i << ":";

    if (not holds(*atom)) { ost << "(not "; }

    problem->print(ost, *atom);

    if (not holds(*atom)) { ost << ")"; }
  }
