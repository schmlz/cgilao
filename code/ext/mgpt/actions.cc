...
VecRationals conditionalEffectList_t::costVector(state_t const& s) const {
  static const VecRationals all_zeros(gpt::problem->domain().functions().size(), 0);
  if (size() == 0) { return all_zeros; }

  bool cond_applicable = false;
  VecRationals rv = effect(0).costVector(s, cond_applicable);
  for (size_t i = 1; i < size(); ++i) {
    cond_applicable = false;
    VecRationals v = effect(i).costVector(s, cond_applicable);
    if (cond_applicable) {
...
VecRationals probabilisticEffectList_t::costVector(state_t const& s) const {
  DIE(size() > 0, "Unexpected size of probabilisticEffectList_t", 162);
  // TODO(fwt): it's assumed that the cost of the action doesn't depend in the
  // outcome of the action; therefore, the cost of all the effects should be the
  // same and returning the cost of the first effect suffices. This is also a
  // legacy assumption of the code. This could really speed up thing here. For
  // now, I'm implementing as the cost functions depends on the resulting state,
  // i.e., C(s,a,s') and computing E_s'[C(s,a,s')|c,a]
  VecRationals expected_cost(gpt::problem->domain().functions().size(), 0);
  for (size_t i = 0; i < size(); ++i) {
    Rational pr = effect(i).probability();
    VecRationals c_i = effect(i).costVector(s);
    for (size_t j = 0; j < c_i.size(); ++j) {
      expected_cost[j] += pr * c_i[j];
    }
  }
  return expected_cost;
}
...
{
  bool rv = false;
  // NOTE: Delete effects are applied first.
...
deterministicEffect_t::insert_effect( const stripsEffect_t &seff )
{
  ...
  s_effect().increaseCostVecBy(seff.costVector());
}
    ...
    state_t const& s, HashAtomtToRational& prob) const
{
  ...
  for (size_t i = 0; i < add.size(); ++i) {
    // for assert only
    [[maybe_unused]] HashAtomtToRational::iterator it = prob.find(add.atom(i));
...
VecRationals deterministicAction_t::costVector() const {
  if (gpt::use_action_cost) {
    VecRationals rv = effect_.costVector();
    // FWT: Cost is always 1 - reward and reward <= 0
    rv[0] = Rational(1) - rv[0];
    if (self_loop_correction_ != 1) {
      for (Rational& c : rv) {
        c *= self_loop_correction_;
      }
    }
    return rv;
  }
  else {
    return VecRationals(gpt::problem->domain().functions().size(),
                        self_loop_correction_);
  }
}
...
VecRationals deterministicAction_t::costVector(state_t const& s) const {
  if (gpt::use_action_cost) {
    VecRationals rv = effect_.costVector(s);
    // FWT: Cost is always 1 - reward and reward <= 0
    rv[0] = Rational(1) - rv[0];
    if (self_loop_correction_ != 1) {
      for (Rational& c : rv) {
...
VecRationals probabilisticAction_t::costVector() const {
  if (gpt::use_action_cost) {
    VecRationals rv = effect_list_.costVector();
    // FWT: Cost is always 1 - reward and reward <= 0
    rv[0] = Rational(1) - rv[0];
    return rv;
  }
  else {
    return VecRationals(gpt::problem->domain().functions().size(), 1);
  }
}
...
VecRationals probabilisticAction_t::costVector(state_t const& s) const {
  if (gpt::use_action_cost) {
    VecRationals rv = effect_list_.costVector(s);
    // FWT: Cost is always 1 - reward and reward <= 0
    rv[0] = Rational(1) - rv[0];
    return rv;
  }
