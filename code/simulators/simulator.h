...
class RoundSummary {
 public:
  ...
  RoundSummary() : accumulatedCost(1, Rational(0)), totalActionsApplied(0),
      totalCpuPlusSystemTime(0),  exitStatus(EndOfRoundStatus::PLANNER_GAVE_UP)
  { }

  RoundSummary(size_t num_cost_f, uint32_t t, uint64_t cpu, EndOfRoundStatus s)
    : accumulatedCost(num_cost_f, Rational(0)), totalActionsApplied(t),
      totalCpuPlusSystemTime(cpu), exitStatus(s)
  { }
  ~RoundSummary() { }

  VecRationals accumulatedCost;
...
class Simulator {
 public:
  Simulator(ConstrSSPIface const& cssp, bool output_turns = false,
            bool save_policy = false) : cssp_(cssp),
...
Simulator* createSimulator(ConstrSSPIface const& ssp, std::string const& name);

class LocalSimulator : public Simulator {
 public:
  LocalSimulator(ConstrSSPIface const& cssp, bool output_turns = false,
            bool save_policy = false) : Simulator(cssp, output_turns,
                                                  save_policy) { }
