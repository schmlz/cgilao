...
/*
 * Represents an Constrained SSP = <S, s_0, G, A, P, vec{C}, vec{C_t}> where:
 *
 *  - S: set of states
 ...
 *  - vec{C}: S x A -> (R*+)^n: the vector cost of applying action a on state s.
 *  - vec{C_t}: S -> R+^n: the vector cost of reaching a state s \in G. C_t(s) >= 0.
 *
 */
class ConstrSSPIface {
 public:
  virtual ~ConstrSSPIface() { }
  ...
  virtual VecRationals costVector(state_t const& s, action_t const& a) const = 0;

  // Returns C_t(s). C_t(s) must be greater or EQUAL to 0.
  virtual Rational terminalCost(state_t const& s) const = 0;
//  virtual VecRationals terminalCostVector(state_t const& s, action_t const& a) const = 0;
  ...
  virtual ConstraintMap const& constraints() const = 0;

  // Return the number of cost functions for this problems, i.e., the dimension
  // of the cost vector
  virtual size_t numCostFunctions() const = 0;
};
 ...
 *
 */
class SSPIface : public ConstrSSPIface {
 public:
  virtual ~SSPIface() { }

  // SSPs have a single cost function
  virtual Rational cost(state_t const& s, action_t const& a) const = 0;

 private:
  /*
   * Hiding the Constraint SSPs methods from the SSP interface
   */
  VecRationals costVector(state_t const& s, action_t const& a) const override {
    // SSPs have a uni-dimensional cost vector
    return VecRationals(1, cost(s, a));
  }

  // SSPs have no constraints
  ConstraintMap const& constraints() const override {
    static const ConstraintMap empty_map;
    return empty_map;
  }

  size_t numCostFunctions() const override {
    // SSPs have only one cost function, the action cost
    return 1;
  }
};
...
class HashsetStateConstIte : public StateConstIteIface {
 public:
  ...
  HashsetStateConstIte(ConstrSSPIface const& ssp, state_t const& s)
    : reachable_states_sptr_(new HashsetState(reachableStatesFrom(ssp, s))),
 ...
 */
inline
StateConstRange wrappedReachableStates(ConstrSSPIface const& ssp, state_t const& s) {
...
class ApplicableActionIterator : public ActionConstIteIface {
 public:
  ApplicableActionIterator(Iterator const& it, Iterator const& end,
                           ConstrSSPIface const& ssp, state_t const& s)
