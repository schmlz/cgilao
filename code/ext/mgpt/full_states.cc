    ...
    std::deque<hashEntry_t*> &space )
{
  ...
  hashEntry_t *entry = hash_table.get(problem.get_initial_state());
  entry->set_bits(OPEN);
  Q.push_front(entry);
    ...
    for( ai = problem.actionsT().begin(); ai != problem.actionsT().end(); ++ai )
      if( (*ai)->enabled( *entry->state() ) )
      {
        ...
        for (auto const& ip : pr) {
          DIE(i < DISP_INT_SIZE, "BUG_1: i >= DISP_INT_SIZE", 201); //BUG_1
          if (!(tmp = hash_table.find(ip.event()))) {
            tmp = hash_table.insert(ip.event());
            tmp->set_bits( OPEN );
            Q.push_front( tmp );
          }
...
state_t::state_t( const problem_t &problem )
{
  ...
  make_digest();
  DIE(make_check(), "Fail state::make_check()", 103);
}
...
state_t::insert_predicate( ushort_t p, ushort_t *ptr )
{
  ...
  DIE(check(nsize_, ndata_), "Fail state::check()", 113);
...
state_t::clear( ushort_t atom )
{
  ...
  if( find_predicate( atom, &ptr ) )
  {
    DIE((data_ < (char*)ptr) && (&data_[size_] > (char*)ptr) && (*ptr == atom), "Fail condition to state_t::clear", 114);
...
void
state_t::full_print( std::ostream &os, const problem_t *problem, bool print_idx ) const
{
  os << "[" << size_ << ":" << digest_ << "]";
  for( ushort_t *p = predicates( data_ ); p < (ushort_t*)fluents( data_ ); ++p )
  {
