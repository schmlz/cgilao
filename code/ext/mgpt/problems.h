...
class problem_t {
  ...
  public:
   ...
   public:
    ...
    const StateFormula& original_goal( void ) const { return( *original_goal_ ); }
    ...
    * FWT: CONSTRAINTS
    *
    * The constraints are in the form E[f] <= c and constraints_ is a mapping
    * from the index of the function f and to the constant c.
    */
   private:
    ConstraintMap constr_map_;
   public:
    void parseConstraints();
    ConstraintMap const& constraints() const { return constr_map_; }
    size_t numCostFunctions() const { return domain().functions().size(); }
   /**************************************************************************/
