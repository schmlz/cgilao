...
PlannerLRTDP::PlannerLRTDP(SSPIface const& ssp, heuristic_t& heur,
  ...
  : OptimalPlanner(), ssp_(ssp), internal_v_(new hash_t(gpt::initial_hash_size, heur)),
    ...
    use_as_replanner_(use_as_replanner)
{ }
...
void PlannerLRTDP::trial(state_t const& s) {
  ...
  while (true) {
...
#ifdef DEBUG_TRACE
    std::cout << "|trace| = " << trace_size << " Trial cur: "
    ...
    // Not using 'a_greedy->affect(cur_s)' because it used the PPDDL action
    // instead of the action from ssp_. It wasn't a problem before since no
    // algorithm was changing actions but now FRET-S3P does it. So, this ensure
    // we are sampling from the correct SSP.
    cur_s = sampleOutcome(cur_s, *a_greedy);
