(define (domain elevators)
  ...
  (:action go-up
    ...
    :effect (and (decrease reward 0) (in ?e ?nf) (not (in ?e ?f)))
  )
  (:action go-down
    :parameters (?e - elevator ?f ?nf - floor)
  ...
  (:action step-in
    ...
    :effect (and (decrease reward 0) (inside ?e) (not (at ?f ?p)))
  )
  (:action step-out
    :parameters (?e - elevator ?f - floor ?p - pos)
    :precondition (and (inside ?e) (in ?e ?f) (shaft ?e ?p))
    :effect (and (decrease reward 0) (at ?f ?p) (not (inside ?e)))
  )
  (:action move-left
    :parameters (?f - floor ?p ?np - pos)
    :precondition (and (at ?f ?p) (dec_p ?p ?np) (not (gate ?f ?p)))
    :effect (and (decrease reward 0) (not (at ?f ?p)) (at ?f ?np))
  )
  (:action move-left-gate
    :parameters (?f - floor ?p ?np - pos)
    :precondition (and (at ?f ?p) (dec_p ?p ?np) (gate ?f ?p))
    :effect (and (decrease reward 0) (probabilistic 1/2 (and (not (at ?f ?p)) (at ?f ?np))
  ...
  (:action collect
    ...
    :effect (and (decrease reward 0) (have ?c) (not (coin-at ?c ?f ?p)))
  )
)
